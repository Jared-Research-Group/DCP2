...
def getLemboxData(f, n = 1000, forceDataUpdate=False):
    ...
    logger.info('Reading LEMBOX data...')

    f = Path(f)
    df = pd.read_csv(f)

    if not 'Scaled_Current(A)' in df.columns:
        df = scale_lembox(f.parent)

    if not 'Avg_Voltage(V)' in df.columns or forceDataUpdate:
        df['Avg_Voltage(V)'] = helper_functions.getRollingAvg(df['Scaled_Voltage(V)'], n)
        df['Avg_Current(A)'] = helper_functions.getRollingAvg(df['Scaled_Current(A)'], n)
